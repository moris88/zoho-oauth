...
export async function actionGenerateToken(
  ...
  try {
    ...
    const redirect_uri = queryData.get("redirect_uri");
...
export async function actionOAuth(
  ...
  const oauth_url = `${baseUrl}?response_type=${responseType}&client_id=${clientId}&redirect_uri=${encodeURIComponent(
    redirect_uri
...
export async function actionRequest(
  ...
  console.log("fetching", method, baseUrl, endpoint, body, headers, params);
  try {
    if (baseUrl && endpoint) {
    ...
    console.error("Error in actionRequest:", error);
    return {
      ...prevState,
      method,
      baseUrl,
